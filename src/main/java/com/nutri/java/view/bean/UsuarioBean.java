...
public class UsuarioBean {
	...
	private UICalendar nasctoBinding;
	...
	public UICalendar getNasctoBinding() {
		return nasctoBinding;
	}

	public void setNasctoBinding(UICalendar nasctoBinding) {
	...
	private static Usuario getLoggedUser() {
		...
		if (sessao != null && usuarioLogado != null) {
			...
			msg.setSummary("� necess�rio efetuar logon para utilizar a aplica��o");
		...
		try {
			...
			if (usuarioService.validarLogin(login, senha)) {
				...
				if (usuarioService.validarUsuario(usuario)) {
					...
					// Limpando os campos associados ao usu�rio
					this.login = null;
					this.senha = null;
					...
					FacesMessage msg = new FacesMessage();
					msg.setSummary("Prezado(a) nutricionista, o seu cadastro ainda n�o foi examinado para lhe permitir acesso. Por favor tente novamente mais tarde.");
				...
				FacesMessage msg = new FacesMessage();
				msg.setSummary("Usu�rio e/ou senha inv�lidos");
				msg.setSeverity(FacesMessage.SEVERITY_INFO);
	...
	public String efetuarLogoff() {
		FacesContext ctx = FacesContext.getCurrentInstance();
		FacesMessage msg = new FacesMessage();
		msg.setSummary("Usu�rio desconectado com sucesso");
		msg.setSeverity(FacesMessage.SEVERITY_INFO);
